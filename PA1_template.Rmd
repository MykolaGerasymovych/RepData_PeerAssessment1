...
### Load packages
For the analysis we'll need several packages that will help to work with dates, plots and also to transform data sets. These are lubridate, ggplot2, grid, gridExtra, dplyr, Scale, knitr

### Load data sets
After we are sure that folder with data is be in the working directory we can load the data for the analysis
...
max_steps_taken; max_steps_interval
```
Consequently, the maximum of 206 steps were taken during the 835-th 5-minute interval or at 8:35-40 o'clock, more likely when going to the job place in the morning.
...
new_mean_steps_per_day; new_median_steps_per_day
```
...
head(avg_steps_by_interval_new); tail(avg_steps_by_interval_new)
```
...
head(weekday_avg_steps_by_interval); head(weekend_avg_steps_by_interval)
```
